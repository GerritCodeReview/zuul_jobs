...
def run(gerrit_project_mapping, gerrit_project_name, gerrit_root, zuul):
    ...
    for project in zuul["projects"].values():
        ...
        try:
            gitmodules_path = os.path.join(project["src_dir"], ".gitmodules")
            runner.log("Reading gitmodules from {0}".format(gitmodules_path))
            config.read(gitmodules_path)
            section_name = "submodule {project_dest}".format(project_dest=project_dest)
        except Exception as e:
            runner.log(str(e))
        if config:
            try:
                project_in_gitmodules = config.get(section_name, "path")
                runner.log("Project Submodule: {sub}".format(sub=project_in_gitmodules))
            except Exception as e:
                runner.log(str(e))
            try:
                tracking_branch = config.get(section_name, "branch")
                runner.log("Tracking Branch: {sub}".format(sub=project_in_gitmodules))
            except Exception as e:
                runner.log(str(e))
        ...
        if project_in_gitmodules and (not project_branch_exists or not tracking_branch):
            runner.run(
                "git submodule update --init {project_dest}".format(
