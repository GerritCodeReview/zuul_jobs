...
class Runner:
    ...
    def run(self, cmd):
        try:
            self.output.append("Running: {0}".format(cmd))
            out = subprocess.check_output(
                [os.path.expanduser(f) for f in shlex.split(cmd)]
            )
...
def run(gerrit_project_mapping, gerrit_project_name, gerrit_root, zuul):
    ...
    # filesystem paths.
    runner.log("cd to: {0}".format(gerrit_root))
    os.chdir(gerrit_root)
    runner.run("git remote rm origin")

    # Read the submodules info from the gerrit repo
    config = configparser.ConfigParser()
    # TODO(mordred): Remove this next section when we've updated to
    # python3. The python2 on stretch doesn't understand leading spaces
    # in ini files.
    gitmodules_content = "\n".join([
        f.strip() for f in open('.gitmodules', 'r').readlines()
    ])
    runner.log(".gitmodules:")
    runner.log(gitmodules_content)
    with tempfile.NamedTemporaryFile() as fp:
        fp.write(gitmodules_content)
        fp.flush()
    try:
        config.read(fp.name)
    except Exception as e:
    ...
    runner.log("Config sections: {0}".format(list(config.sections())))
    ...
    for project in zuul["projects"].values():
        ...
        # Prepare Project
        runner.log("Analyzing project {name} with dest {dest}".format(name=project["name"], dest=project_dest))

        # If zuul checked out the branch we're testing (ie, gerrit's
        # branch), then it exists.
        project_branch_exists = (
            zuul["projects"][gerrit_project_name]["checkout"] == project["checkout"]
        )
        ...
        section_name = 'submodule "{0}"'.format(project_dest)

        try:
            runner.log("Getting path from {0}".format(section_name))
            project_in_gitmodules = config.get(section_name, "path")
            runner.log("Project Submodule: {0}".format(project_in_gitmodules))
        except Exception as e:
            runner.log(str(e))
        try:
            runner.log("Getting branch from {0}".format(section_name))
            tracking_branch = config.get(section_name, "branch")
            runner.log("Tracking Branch: {0}".format(tracking_branch))
        except Exception as e:
            runner.log(str(e))
        ...
        if (
            ...
            message = textwrap.dedent(
                ...
                branch in the {project_name} repo.  Alternatively, you can
                merge the dependent change, manually update the submodule
                pointer, then test this change again.""".format(
        ...
        if project_in_gitmodules and (not project_branch_exists or not tracking_branch):
            runner.run(
                "git submodule update --init {0}".format(project_dest)
            )

        else:
            runner.run(
                "mv -T -f ~/{project_src_dir}"
                " {gerrit_root}/{project_dest}".format(
...
def ansible_main():
    ...
    try:
        ...
        module.exit_json(changed=True, debug_out=debug_out)
    except Exception as e:
